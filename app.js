...
function saveContact(){if(!stop())return;contacts[stop().id]={owner:document.querySelector('#ownerInput').value.trim(),phone:document.querySelector('#phoneInput').value.trim(),email:document.querySelector('#emailInput').value.trim(),instagram:document.querySelector('#instaInput').value.trim(),bestTime:document.querySelector('#timeInput').value.trim()};localStorage.setItem(contactsKey,JSON.stringify(contacts));document.querySelector('#saveContactBtn').textContent='Saved';cloudPush();}
